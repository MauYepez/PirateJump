import os

readme_content = """# 🏴‍☠️ PirateJump

A charming 2D platformer built with **Unity** where you take on the role of a daring pirate on a quest for legendary treasure. Avoid hazards, perfect your jumps, and claim your pirate glory!

---

## 🎮 Game Objective

The rules of the sea are simple but challenging:
1. **Explore** the treacherous island layout.
2. **Collect all 3 hidden Gems** scattered across the map.
3. **Reach the Pirate Flag** to secure your treasure and win the game!

*Note: The victory flag will only activate once all 3 gems have been successfully collected.*

---

## 🕹️ Controls

Set sail with these simple, responsive controls designed for precise platforming:

| Action | Primary Key | Alternative Key |
| :--- | :--- | :--- |
| **Move Left** | `A` | `Left Arrow` |
| **Move Right** | `D` | `Right Arrow` |
| **Jump** | `Spacebar` | `Spacebar` |

---

## 🚀 Installation & How to Play

### Playing the Release Build
1. Download the latest game build from the [Releases](../../releases) section.
2. Extract the ZIP file to your local directory.
3. Run `PirateJump.exe` (Windows) or the corresponding executable for your OS.

### Running the Source Code in Unity
1. Clone this repository to your local machine:
